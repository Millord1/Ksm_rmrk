                ...
                if (lockExists) {
                    ...
                    jetski.getBlockContent(blockNumber, api)
                        ...
                        if (rmrksWithMeta.length > 0) {
                            ...
                            const blockchain = GossiperFactory_1.GossiperFactory.getCanonizeChain(chain, canonizeManager.getSandra());
                            let gossip;
                            let gossiper;
                            let i = 0;
                            let sent = false;
...
async function eggs(opts, counter, blockN, jetski, api) {
    ...
    jetski.getBigBlock(block, api, count)
        ...
        console.log(count + " " + count * Jetski_1.Jetski.maxPerBatch);
    }).catch((e) => {
        console.error(e);
    });
}
...
async function sendWithDelay(canonizeManager, block, blockchain) {
